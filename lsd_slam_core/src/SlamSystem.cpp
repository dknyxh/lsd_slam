...
void SlamSystem::randomInit(uchar* image, uchar* mask, double timeStamp, int id){
	...
	currentKeyFrame.reset(new Frame(id, width, height, K, timeStamp, image, mask));
...
void SlamSystem::trackFrame(uchar* image, uchar* mask, unsigned int frameID, bool blockUntilMapped, double timestamp)
{
	// Create new frame
	std::shared_ptr<Frame> trackingNewFrame(new Frame(frameID, width, height, K, timestamp, image, mask));
